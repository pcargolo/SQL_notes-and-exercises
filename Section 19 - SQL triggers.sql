...
# exercise 243 - Create a trigger that checks if the hire date of an employee is higher than the current date. If true, set this date to be the current date. Format the output appropriately (YY-MM-DD).
DELIMITER $$

CREATE TRIGGER trig_new_hire
BEFORE UPDATE ON employees
FOR EACH ROW
BEGIN 
	IF NEW.hire_date > DATE(NOW()) THEN -- IF NEW.hire_date > date_format(sysdate(), '%Y-%m-%d') THEN
		SET NEW.hire_date = DATE(NOW()); -- SET NEW.hire_date = date_format(sysdate(), '%Y-%m-%d'); 
	END IF; 
END $$
